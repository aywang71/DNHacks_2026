...
Reference each section using only the provided source IDs. Suggested checks are proposals, never completed findings. Write prose without Markdown syntax or HTML inside text fields. Prefer short paragraphs and concrete language. Do not mention JSON fields, objects, or implementation details. Express priority scores out of 100 by multiplying the supplied 0–1 scores by 100. Offer three useful follow-up questions answerable from the data or its limitations.`
...
export async function generate({ mode, context, messages = [], apiKey, model = 'gemini-3.6-flash', fetcher = fetch, signal }) {
  if (!apiKey) throw new HttpError(503, 'AI is not configured. Add GEMINI_API_KEY to the server environment and restart the backend.')
  ...
  try {
    response = await fetcher(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
      ...
      body: JSON.stringify({ systemInstruction: { parts: [{ text: instructions }] },
        contents: input.map(message => ({ role: message.role === 'assistant' ? 'model' : 'user', parts: [{ text: message.content }] })),
        generationConfig: { maxOutputTokens: 6000, responseMimeType: 'application/json', responseJsonSchema: outputSchema(sourceIds) } }),
    })
  } catch (error) {
  ...
  if (!response.ok) {
    ...
    const invalidKey = details.error?.details?.some(detail => ['API_KEY_INVALID', 'API_KEY_EXPIRED'].includes(detail.reason))
    const message = invalidKey || response.status === 401 || response.status === 403 ? 'Gemini rejected the server credentials. Check GEMINI_API_KEY and model access, then restart the backend.'
  ...
  try {
    ...
    if (candidate?.finishReason !== 'STOP' || payload.promptFeedback?.blockReason) throw new Error('Incomplete or blocked response')
    result = JSON.parse(candidate.content.parts.filter(part => typeof part.text === 'string' && !part.thought).map(part => part.text).join(''))
