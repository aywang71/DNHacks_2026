...
export function VesselPanel({ vessels, selectedId, positions, visibleIds, ready, validRange, indexReady, indexError, rangeKey, onSelect, onRetry, onJump }: Props) {
  ...
  useEffect(() => { if (selectedId) selectedHeading.current?.focus() }, [selectedId])
  ...
  return <aside className="vessel-panel" aria-label="Vessels in selected range">
    ...
    <label className="vessel-search" htmlFor="vessel-search"><span>Search vessels</span><input ref={search} id="vessel-search" name="vessel-search" type="search" aria-label="Search vessels" placeholder="Name, MMSI, IMO, or GFW ID" value={query} onChange={event => setQuery(event.target.value)} /></label>
    {selectedId && <section className="vessel-detail" aria-label="Selected vessel details"><div className="detail-title"><h2 ref={selectedHeading} tabIndex={-1}>{selected ? label(selected) : 'Selected vessel'}</h2><button aria-label="Close vessel details" onClick={() => { onSelect(null); search.current?.focus() }}>×</button></div>{selected ? <>
