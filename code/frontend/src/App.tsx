...
export default function App() {
  ...
  const openCase = () => {
    setCaseRecord(current => ({ ...current, status: current.status === 'not-created' ? 'open' : 'ready', createdAt: current.createdAt ?? '05 Sep 2026, 12:08 UTC' }))
    setNotice(caseRecord.status === 'not-created' ? `Investigation ${caseRecord.id} is open.` : 'Evidence brief marked ready.')
  }
  return <div className="app-shell">
    ...
    <main id="workspace" className={`workspace ${inspectorOpen ? 'inspector-open' : 'inspector-closed'}`}><aside className="rail" aria-label="Workspace navigation"><p>Workspace</p><button>Overview</button><button className="active" aria-current="page">Risk map</button><button>Investigations <b>12</b></button><button>Watchlists</button><button>Reports</button><section><p>Data status</p><span className="data-status">Offline-ready<br />curated replay</span></section></aside><section className="map-region" aria-label="Maritime replay map"><MapPanel selected={selected} onSelectVessel={(id) => { setSelectedId(id); setInspectorOpen(true) }} />{query && matching.length > 0 && <div className="search-results" aria-label="Matching vessels">{matching.map(v => <button key={v.id} aria-current={v.id === selectedId || undefined} onClick={() => { setSelectedId(v.id); setInspectorOpen(true); setQuery('') }}>{v.name} <small>{v.imo}</small></button>)}</div>}</section><InvestigationPanel vessels={vessels} selected={selected} onSelect={(id) => { setSelectedId(id); setInspectorOpen(true) }} isOpen={inspectorOpen} onClose={() => setInspectorOpen(false)} caseRecord={caseRecord} onOpenCase={openCase} onSetNotes={notes => setCaseRecord(c => ({ ...c, notes }))} watching={watching} onWatch={() => { setWatching(x => !x); setNotice(watching ? 'Removed from watchlist.' : 'Added to watchlist.') }} filter={filter} onFilter={setFilter} /></main>
