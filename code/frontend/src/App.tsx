...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main id="workspace" className="workspace"><aside className="rail" aria-label="Workspace navigation"><p>Workspace</p><button>Overview</button><button className="active" aria-current="page">Risk map</button><button>Investigations <b>12</b></button><button>Watchlists</button><button>Reports</button><section><p>Data status</p><span className="data-status">Offline-ready<br />curated replay</span></section></aside><section className="map-region" aria-label="Maritime replay map"><MapPanel selected={selected} onSelectVessel={setSelectedId} />{query && matching.length > 0 && <div className="search-results" aria-label="Matching vessels">{matching.map(v => <button key={v.id} aria-current={v.id === selectedId || undefined} onClick={() => { setSelectedId(v.id); setQuery('') }}>{v.name} <small>{v.imo}</small></button>)}</div>}</section><InvestigationPanel vessels={vessels} selected={selected} onSelect={setSelectedId} caseRecord={caseRecord} onOpenCase={openCase} onSetNotes={notes => setCaseRecord(c => ({ ...c, notes }))} watching={watching} onWatch={() => { setWatching(x => !x); setNotice(watching ? 'Removed from watchlist.' : 'Added to watchlist.') }} filter={filter} onFilter={setFilter} /></main>
