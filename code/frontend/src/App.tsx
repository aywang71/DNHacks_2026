...
export default function App() {
  ...
  useEffect(() => {
    ...
    }).catch(error => { if (!abort.signal.aborted) setRiskError(error instanceof Error ? error.message : 'The GapPair export could not be loaded.') })
    return () => abort.abort()
  }, [riskRetry])
  useEffect(() => {
    const abort = new AbortController()
