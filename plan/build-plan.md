...
> **Historical implementation note.** This design predates the current Python
> ingestion CLI, GFW Bronze/Silver collections, Node presence importer, and
> frontend viewer. Treat the sections below as a detection proposal, not a
> statement of the live repository state. For current locations and operational
> boundaries, use [REPO_MAP.md](../REPO_MAP.md),
> [AIS ingestion architecture](../docs/ais-ingestion.md), and
> [the Bronze runbook](../docs/bronze-backfill-runbook.md).
...
## 8. Future candidate-detection stages and files

The table below is a proposed detection pipeline, not the current ingestion
implementation. Existing provider adapters live in `src/dark_rendezvous/` and
already use `httpx` and `shapely`; see [REPO_MAP.md](../REPO_MAP.md). Future
candidate code should use a dedicated package rather than resurrecting the
obsolete `pipeline/` paths named below.
